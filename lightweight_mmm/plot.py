...
@functools.partial(jax.jit, static_argnames=("media_mix_model", "target_scaler"))
def _generate_diagonal_predictions(
    ...
    make_predictions = jax.vmap(
        fun=_make_single_prediction, in_axes=(None, 0, None, None)
    )
    ...
    predictions = (
        make_predictions(media_mix_model, diag_media_values, extra_features, seed)
        - prediction_offset
    )
...
def create_media_baseline_contribution_df(
    ...
    sum_scaled_media_contribution_across_channels_samples = (
        sum_scaled_media_contribution_across_channels.sum(axis=0)
    )
    ...
    adjusted_sum_scaled_baseline_contribution_across_samples = np.where(
        sum_scaled_baseline_contribution_across_samples < 0,
        0,
    ...
    media_contribution_pct_by_channel = np.nan_to_num(media_contribution_pct_by_channel)
    ...
    baseline_contribution_pct = np.nan_to_num(baseline_contribution_pct)
    ...
    posterior_pred_df["avg_prediction"] = np.where(
        posterior_pred_df["avg_prediction"] < 0, 0, posterior_pred_df["avg_prediction"]
    )
    ...
    for channel in channel_names:
        ...
        contribution_df.loc[:, channel_contribution_col_name] = (
            contribution_df[channel_pct_col] * contribution_df["avg_prediction"]
        )
        contribution_df.loc[:, channel_contribution_col_name] = contribution_df[
            channel_contribution_col_name
        ].astype("float")
    contribution_df.loc[:, "baseline contribution"] = (
        contribution_df["baseline_percentage"] * contribution_df["avg_prediction"]
    )
...
def plot_response_curves(
    ...
    make_predictions = jax.vmap(
        jax.vmap(_make_single_prediction, in_axes=(None, 0, None, None), out_axes=0),
        in_axes=(None, 0, None, None),
        out_axes=1,
    )
    diagonal = jnp.repeat(
        jnp.eye(media_mix_model.n_media_channels), steps, axis=0
    ).reshape(media_mix_model.n_media_channels, steps, media_mix_model.n_media_channels)

    prediction_offset = media_mix_model.predict(
        media=jnp.zeros((1, *media.shape[1:])), extra_features=extra_features
    ).mean(axis=0)
    ...
    predictions = jnp.squeeze(
        a=make_predictions(media_mix_model, mock_media, extra_features, seed)
    )
    ...
    fig = plt.figure(
        media_mix_model.n_media_channels + 1, figsize=figure_size, tight_layout=True
    )
    ...
    last_ax.set_xlabel(
        "Normalized spend per channel" if not media_scaler else "Spend per channel"
    )
    plt.close()
    return fig
...
def plot_cross_correlate(
    feature: jnp.ndarray, target: jnp.ndarray, maxlags: int = 10
) -> Tuple[int, float]:
    ...
    plot = plt.xcorr(x=feature - mean_feature, y=target - mean_target, maxlags=maxlags)
    plt.show()
...
def plot_var_cost(
    media: jnp.ndarray, costs: jnp.ndarray, names: List[str]
) -> matplotlib.figure.Figure:
...
def _create_shaded_line_plot(
    ...
    if predictions.shape[1] != len(target):
        raise ValueError("Predicted data and ground-truth data must have same length.")
    ...
    axis.plot(jnp.arange(target.shape[0]), target, c="grey", alpha=0.9)
    axis.plot(
        jnp.arange(target.shape[0]), predictions.mean(axis=0), c="green", alpha=0.9
    )
    axis.fill_between(
...
def _call_fit_plotter(
    predictions: jnp.array, target: jnp.array, interval_mid_range: float, digits: int
) -> matplotlib.figure.Figure:
    ...
    if predictions.ndim == 3:  # Multiple plots for geo model
        figure, axes = plt.subplots(
            predictions.shape[-1], figsize=(10, 5 * predictions.shape[-1])
        )
...
def plot_media_channel_posteriors(
    ...
    n_geos = media_mix_model.media.shape[2] if media_mix_model.media.ndim == 3 else 1
    ...
    fig, axes = plt.subplots(nrows=n_media_channels, ncols=n_geos, figsize=fig_size)
...
def plot_bars_media_metrics(
    ...
    quantile_bounds = np.quantile(metric, q=[lower_quantile, upper_quantile], axis=0)
...
def plot_pre_post_budget_allocation_comparison(
    ...
    axes[0].set_title(
        "Before and After Optimization Budget Allocation Comparison", fontsize="x-large"
    )
    ...
    plots3 = axes[1].bar(
        ["pre optimization predictited target", "post optimization predicted target"],
        predictions,
    )
    axes[1].set_ylim(
...
def _make_prior_and_posterior_subplot_for_one_feature(
    ...
    prior_samples = prior_distribution.sample(
        key=jax.random.PRNGKey(seed=seed), sample_shape=(number_of_samples_for_prior,)
    )
    ...
    if hyperprior:
        ...
        prior_samples = prior_distribution.sample(
            ...
            sample_shape=(square_root_of_number_of_samples_for_prior,),
        ).flatten()
    ...
    ax.legend(loc="best")
    ax.set_xlim(
        min(prior_xlims[0], posterior_xlims[0]), max(prior_xlims[1], posterior_xlims[1])
    )
    ax.set_yticks([])
    ax.set_ylabel("")
...
def _collect_features_for_prior_posterior_plot(
    ...
    if not all(
        [hasattr(media_mix_model, x) for x in media_mix_model_attributes_to_check_for]
    ):
        raise lightweight_mmm.NotFittedModelError(
    ...
    other_features = list(
        set(features)
        - set(geo_level_features)
        - set(channel_level_features)
...
def plot_prior_and_posterior(
    ...
    (
        ...
        seasonal_features,
        other_features,
    ) = _collect_features_for_prior_posterior_plot(media_mix_model, selected_features)

    number_of_subplots = int(
        sum([np.product(media_mix_model.trace[x].shape[1:]) for x in features])
    )
    ...
    i_ax = 0
    for feature in (
        geo_level_features + channel_level_features + seasonal_features + other_features
    ):
            ...
            for i_feature in range(media_mix_model.trace[feature].shape[1]):
                for j_geo in range(media_mix_model.n_geos):
                    ...
                    (
                        ...
                        gridspec_fig,
                        i_ax,
                    ) = _make_prior_and_posterior_subplot_for_one_feature(
        ...
        if feature in seasonal_features:
            for i_season in range(media_mix_model._degrees_seasonality):
                for j_season in range(2):
                    ...
                    posterior_samples = np.array(
                        media_mix_model.trace[feature][:, i_season, j_season]
                    )
                    (
                        fig,
                        gridspec_fig,
