...
def _budget_constraint(
    media: jnp.ndarray, prices: jnp.ndarray, budget: jnp.ndarray
) -> jnp.float64:
...
def _get_lower_and_upper_bounds(
    ...
    return optimize.Bounds(
        lb=lower_bounds * n_time_periods, ub=upper_bounds * n_time_periods
    )
...
def _generate_starting_values(
    ...
    print(f"Using only {n_time_periods} weeks to generate starting values")
    # previous_allocation = media[-n_time_periods:].mean(axis=0) * n_time_periods
...
def find_optimal_budgets(
    ...
    media_scaler: Optional[preprocessing.CustomScaler] = None,
    bounds_lower_pct: Union[float, jnp.ndarray] = 0.2,
    bounds_upper_pct: Union[float, jnp.ndarray] = 0.2,
    ...
    if hasattr(media_mix_model, "media_data_test"):
        print("found media_data_test in MMM, using it for optimization constraints")
        media = media_mix_model.media_data_test
    else:
    ...
    if media_mix_model.n_geos == 1:
        ...
        average_per_time = media.mean(axis=0)
        geo_ratio = average_per_time / jnp.expand_dims(
            average_per_time.sum(axis=-1), axis=-1
        )
    media_input_shape = (n_time_periods, *media.shape[1:])
    ...
    solution = optimize.minimize(
        ...
        constraints={"type": "eq", "fun": _budget_constraint, "args": (prices, budget)},
    )

    kpi_without_optim = _objective_function(
        extra_features=extra_features,
